...
const BEEP_BOOP_STAKE_CONTRACT_TX: u128 = 831;
...
impl Claim {
    ...
    pub fn get_total_available_to_claim(&self, block: u128, tx: u128) -> Result<CallResponse> {
        ...
        let alkane_id = AlkaneId { block, tx };
        let original_nft_id = self
            .resolve_alkane_id(&alkane_id)
            .map_err(|e| anyhow!("Failed to resolve alkane ID {}:{}: {}", block, tx, e))?;
    ...
    fn calculate_total_rewards(&self, alkane_id: &AlkaneId, is_original: bool) -> Result<u128> {
        let total_staked_blocks = self.get_total_staked_blocks_from_contract(alkane_id)?;

        if is_original {
            return Ok(total_staked_blocks);
        }

        let current_staking_blocks = self.get_current_staking_period(alkane_id)?;
    ...
    pub fn get_staked_height(&self, block: u128, tx: u128) -> Result<CallResponse> {
        ...
        let staked_at_block = 19000u128;
        response.data = staked_at_block.to_le_bytes().to_vec();

        Ok(response)
    }

    pub fn get_current_block_height(&self) -> Result<CallResponse> {
        let context = self.context()?;
        let mut response = CallResponse::forward(&context.incoming_alkanes);
        response.data = self.height().to_le_bytes().to_vec();
        Ok(response)
    }
    ...
    fn get_staked_by_lp(&self, block: u128, tx: u128) -> Result<CallResponse> {
        ...
        let alkane_id_string = format!("{}:{}", block, tx);

        response.data = alkane_id_string.as_bytes().to_vec();
    ...
    fn resolve_alkane_id(&self, id: &AlkaneId) -> Result<AlkaneId> {
        ...
        // Check if we got a valid response indicating eligibility
        if response.data.len() >= 1 {
            let eligible = response.data[0];
            if eligible == 1 {
                // NFT is eligible for staking, return the original ID
                return Ok(*id);
            }
        }

        // NFT is not eligible or invalid response, try to get original NFT from LP
